...
function Weapons({ mode = MoveDisplayMode.default }: { mode?: MoveDisplayMode }) {
	...
	return (
		<div className={st.weaponsContainer} data-displaymode={mode}>
			<BlockHeading
				icon={icoCombat}
				label='Weapons'
				...
				{character?.weapons.map((weapon: WeaponDefinition, index: number) => (
					<div className={st.weapon} key={`weapon-${index}`}>
						...
						<div className={st.effects}>
							...
							<button className={st.removeWeaponButton} onClick={() => updateCharacterValue(`weapons.${index}`, '')}>X</button>
						</div>
					</div>
				))}
			</div>

			{character.purchases.weapon_specialisations > 0 && (
				<>
					<BlockHeading
						icon={icoShuriken}
						label='Specialisations'
						className={st.specialisationsHeading}
						addendum={mode != MoveDisplayMode.display && <PurchasePointGroup count={3} columns={3} purchased={character.purchases.weapon_specialisations} maxPurchases={3} purchaseCallback={characterPurchaseUpdater('weapon_specialisations')!} />} />
